...
// Mirrors the bits translate_wire_to_torrent_flags() below handles, so
// set_flags() can tell a real change from a mask that would silently
// translate to an empty torrent_flags_t and no-op.
constexpr wire_flags_t set_flags_known_flags = wire::stopped | wire::auto_managed
	| wire::sequential_download | wire::seed_mode | wire::upload_mode | wire::share_mode
	| wire::super_seeding | wire::disable_pex | wire::disable_dht | wire::disable_lsd
	| wire::disable_v1_hashes | wire::i2p_torrent;
...
{
	// Mirrors set-tag's permission contract so a caller whose
	// allow_set_flags() only covers some bits can still apply those bits,
	// rather than a single out-of-scope bit failing the whole batch. Also
	// gated on set_flags_known_flags so a request denied below is denied
	// for the right reason, not counted as applied.
	std::uint64_t const permitted = std::uint64_t(
		static_cast<std::uint32_t>(st->perms()->allow_set_flags() & aux::set_flags_known_flags)
	);
	...
	for (std::uint16_t i = 0; i < num_entries; ++i) {
		...
		// Excluded from attempted/denied so a batch of pure no-ops never
		// trips the permission_denied threshold below.
		if (wire_mask == 0) continue;

		++attempted;
		std::uint64_t const effective_mask = wire_mask & permitted;
		if (effective_mask == 0) {
			++denied;
			continue;
		}
		...
		ts.handle.set_flags(
			...
			aux::translate_wire_to_torrent_flags(effective_mask)
		);
		++counter;
	}

	// Failing only on total denial lets a reduced allow_set_flags() mask
	// still batch requests across entries with mixed permitted bits.
	if (attempted > 0 && attempted == denied) return error(st, f, permission_denied);
