	...
	torrent_post_limits const& limits)
{
	...
	while (!remaining.empty())
	{
				...
				|| content_type == "application/x-bittorrent"))
		{
			...
			lt::add_torrent_params p = lt::load_torrent_buffer(
				...
				part_ec, lt::load_torrent_limits{});
			if (!part_ec)
	...
	if (result.empty())
	{
		...
		if (found_matching_part && last_part_ec) { ec = last_part_ec; return {}; }
		return invalid();
	}
	return result;
}
...
torrent_post_handler::torrent_post_handler(lt::session& ses
	, auth_interface const* auth
	, save_settings_interface* settings)
	: m_ses(ses)
	, m_auth(auth)
	, m_settings(settings)
{}

std::string torrent_post_handler::path_prefix() const
{
	return "/bt/add";
}

void torrent_post_handler::handle_http(http::request<http::string_body> req
	...
	, std::function<void(bool)> done)
{
	...
	lt::error_code ec;
	auto params = parse_torrent_post(req, ec,
		{100, 100LL * 1024 * 1024, 110LL * 1024 * 1024});

	if (ec)
	{
		auto const status = (ec == lt::error_code(boost::system::errc::file_too_large
	...
	std::string const save_path = m_settings
		? m_settings->get_str("save_path", "./downloads")
		: "./downloads";
