...
struct method_handler {
	...
	void (utorrent_webui::*fun)(
		std::vector<char>&, char const* args, permissions_interface const* p
	);
};
...
get_query_var(std::string_view query, std::string_view var_name)
{
	while (!query.empty()) {
		...
		std::string_view const val = query.substr(
			eq + 1, amp == std::string_view::npos ? std::string_view::npos : amp - eq - 1
		);
...
void utorrent_webui::handle_http(
...
)
{
	...
	std::string const query_string =
		qpos != std::string::npos ? target.substr(qpos + 1) : std::string{};
	...
	if (uri == "/gui/token.html") {
		...
		appendf(
			response, "<html><div id='token' style='display:none;'>%s</div></html>", m_token.c_str()
		);
...
void utorrent_webui::force_start(
	std::vector<char>&, char const* args, permissions_interface const* p
)
{
	if (!p->allow_start()) return;
...
void utorrent_webui::recheck(std::vector<char>&, char const* args, permissions_interface const* p)
{
	...
	apply_fun(args, [](lt::torrent_status const& st) { st.handle.force_recheck(); });
}
...
void utorrent_webui::queue_up(std::vector<char>&, char const* args, permissions_interface const* p)
{
	...
	apply_fun(args, [](lt::torrent_status const& st) { st.handle.queue_position_up(); });
}

void utorrent_webui::queue_down(
	std::vector<char>&, char const* args, permissions_interface const* p
)
{
	if (!p->allow_queue_change()) return;

	apply_fun(args, [](lt::torrent_status const& st) { st.handle.queue_position_down(); });
}
...
void utorrent_webui::queue_top(std::vector<char>&, char const* args, permissions_interface const* p)
{
	...
	apply_fun(args, [](lt::torrent_status const& st) { st.handle.queue_position_top(); });
}

void utorrent_webui::queue_bottom(
	std::vector<char>&, char const* args, permissions_interface const* p
)
{
	if (!p->allow_queue_change()) return;

	apply_fun(args, [](lt::torrent_status const& st) { st.handle.queue_position_bottom(); });
}

void utorrent_webui::remove_torrent(
	std::vector<char>&, char const* args, permissions_interface const* p
)
{
	if (!p->allow_remove()) return;

	apply_fun(args, [this](lt::torrent_status const& st) { m_ses.remove_torrent(st.handle); });
}

void utorrent_webui::set_file_priority(
	std::vector<char>&, char const* args, permissions_interface const* p
)
{
...
void utorrent_webui::remove_torrent_and_data(
	std::vector<char>&, char const* args, permissions_interface const* p
)
{
...
void utorrent_webui::list_dirs(
	std::vector<char>& response, char const* args, permissions_interface const* p
)
{
	std::string const save_path = m_settings.get_str("save_path", "./downloads");
	appendf(
		response,
		", \"download-dirs\": [{\"path\":\"%s\",\"available\":%" PRId64 "}]",
		escape_json(save_path).c_str(),
		free_disk_space(save_path) / 1024 / 1024
	);
}

char const* settings_name(int s) { return lt::name_for_setting(s); }

void utorrent_webui::get_settings(
	std::vector<char>& response, char const* args, permissions_interface const* p
)
{
	...
	for (int i = 0; i < lt::settings_pack::num_string_settings; ++i) {
		...
		appendf(
			...
			"[\"%s\",2,\"%s\",{\"access\":\"%c\"}]\n",
			settings_name(s),
			escape_json(sett.get_str(s)).c_str(),
...
*/
	...
	appendf(
		response,
		"[\"torrents_start_stopped\",1,\"%s\",{\"access\":\"%c\"}]\n",
...
bool to_bool(std::string const& s) { return s != "false" && s != "0"; }

void utorrent_webui::set_settings(
	std::vector<char>& response, char const* args, permissions_interface const* p
)
{
	lt::settings_pack pack;
...
void utorrent_webui::send_file_list(
	std::vector<char>& response, char const* args, permissions_interface const* p
)
{
	...
	for (std::vector<lt::torrent_status>::iterator i = t.begin(), end(t.end()); i != end; ++i) {
		...
		for (lt::file_index_t i : files.file_range()) {
			...
			if (m_version > 0) {
				appendf(response, ", %d, %d]", first_piece, last_piece - first_piece);
			} else {
				response.push_back(']');
			}
		}
...
std::string trackers_as_string(lt::torrent_handle h)
{
	...
	int last_tier = 0;
	for (std::vector<lt::announce_entry>::iterator i = trackers.begin(), end(trackers.end());
		 i != end;
		 ++i) {
...
void utorrent_webui::add_url(std::vector<char>&, char const* args, permissions_interface const* p)
{
	...
	auto const url_val =
		get_query_var(args, "url") ? get_query_var(args, "url") : get_query_var(args, "s");
	if (!url_val) return;
...
void utorrent_webui::get_properties(
	std::vector<char>& response, char const* args, permissions_interface const* p
)
{
	if (!p->allow_list()) return;
...
std::string utorrent_peer_flags(lt::peer_info const& pi)
{
	...
	if (pi.flags & lt::peer_info::optimistic_unchoke) ret += 'O';

	if (pi.flags & lt::peer_info::snubbed) ret += 'S';

	// separate flags from sources with a space
	ret += ' ';

	if (!(pi.source & lt::peer_info::incoming)) ret += 'I';

	if ((pi.source & lt::peer_info::dht)) ret += 'H';

	if ((pi.source & lt::peer_info::pex)) ret += 'X';

	if ((pi.source & lt::peer_info::lsd)) ret += 'L';
	...
	if ((pi.flags & lt::peer_info::on_parole)) ret += 'F';

	if (pi.flags & lt::peer_info::utp_socket) ret += 'P';
	return ret;
}

void utorrent_webui::send_peer_list(
	std::vector<char>& response, char const* args, permissions_interface const* p
)
{
	if (!p->allow_list()) return;
	...
	bool first = true;
	for (std::vector<lt::torrent_status>::iterator i = torrents.begin(), end(torrents.end());
		 i != end;
		 ++i) {
		std::shared_ptr<const lt::torrent_info> ti = i->torrent_file.lock();
		if (!ti || !ti->is_valid()) continue;
		...
		if (!first) response.push_back(',');
		first = false;
		appendf(response, "\"%s\",[", to_hex(i->info_hashes.get_best()).c_str());
		...
		for (lt::peer_info const& p : peers) {
			...
			std::string const ep = addr.is_v6() ? str('[', addr, "]:", p.remote_endpoint().port())
...
void utorrent_webui::get_version(
	std::vector<char>& response, char const* args, permissions_interface const* p
)
{
	...
	appendf(
		...
		to_hex(our_peer_id).c_str(),
		m_ses.get_settings().get_str(lt::settings_pack::user_agent).c_str()
	);
}
...
int utorrent_status(lt::torrent_status const& st)
{
	...
	if (!(st.flags & lt::torrent_flags::paused)
		&& (st.state == lt::torrent_status::downloading
			...
			|| st.state == lt::torrent_status::seeding || st.state == lt::torrent_status::finished))
		ret |= STARTED;
...
std::string utorrent_message(lt::torrent_status const& st)
{
	...
	if (st.state == lt::torrent_status::checking_resume_data) return "Checking";
	...
	if (st.state == lt::torrent_status::seeding || st.state == lt::torrent_status::finished) {
		if (st.flags & lt::torrent_flags::auto_managed) {
	...
	if (st.state == lt::torrent_status::downloading_metadata) return "Downloading metadata";
...
void utorrent_webui::send_torrent_list(
	std::vector<char>& response, char const* args, permissions_interface const* p
)
{
	if (!p->allow_list()) return;
	...
	int cid = 0;
	if (auto const cid_val = get_query_var(args, "cid")) cid = atoi(url_decode(*cid_val).c_str());
	...
	for (torrent_history_entry const& e : r.updated) {
		...
		appendf(
			response,
			"[\"%s\",%d,\"%s\",%" PRId64 ",%d,%" PRId64 ",%" PRId64
			",%f,%d,%d,%d,\"%s\",%d,%d,%d,%d,%d,%d,%" PRId64 "",
			...
			t.download_payload_rate == 0
				? 0
				: (t.total_wanted - t.total_wanted_done) / t.download_payload_rate,
			"" // label
			,
...
void utorrent_webui::send_rss_list(
	std::vector<char>& response, char const* args, permissions_interface const* p
)
{
	if (!p->allow_list()) return;
