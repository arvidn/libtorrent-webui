...
bool websocket_conn::send_packet(std::vector<char>&& packet)
{
	...
	boost::asio::dispatch(
		...
		[self = shared_from_this(), packet = std::move(packet)]() mutable {
			...
			self->m_send_buffer.push_back(std::move(packet));
...
{
	m_conn.async_accept(
		request, beast::bind_front_handler(&websocket_conn::on_accept, shared_from_this())
	);
}
...
void websocket_conn::do_read()
{
	m_read_buffer.clear();
	m_conn.async_read(
		m_read_buffer, beast::bind_front_handler(&websocket_conn::on_read, shared_from_this())
	);
}
...
void websocket_conn::do_close()
{
	...
	m_conn.async_close(
		ws::close_reason{}, beast::bind_front_handler(&websocket_conn::on_close, shared_from_this())
	);
}
